...
function hasClassLikePrototype(value: AnyCallable): boolean {
  const candidate = value as unknown as { prototype?: unknown };
...
function isClassConstructor(value: unknown): value is AnyClass {
  ...
  // Metro/Babel can transpile classes to functions. Detect those by prototype methods.
  return hasClassLikePrototype(value as AnyCallable);
}
