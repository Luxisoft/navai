...
type NavaiRuntimeEnvSource = Record<string, unknown>;
...
export type NavaiMobileRuntimeEnv = Record<string, string | undefined> & {
  NAVAI_API_URL?: string;
  NAVAI_FUNCTIONS_FOLDERS?: string;
  NAVAI_AGENTS_FOLDERS?: string;
  NAVAI_ROUTES_FILE?: string;
  NAVAI_REALTIME_MODEL?: string;
};

export type ResolveNavaiMobileEnvOptions = {
  sources?: Array<NavaiRuntimeEnvSource | undefined>;
};

export type ResolveNavaiMobileRuntimeConfigOptions = {
  moduleLoaders: NavaiFunctionModuleLoaders;
  defaultRoutes: NavaiRoute[];
  env?: NavaiRuntimeEnvSource;
  routesFile?: string;
...
export type ResolveNavaiMobileRuntimeConfigResult = {
  routes: NavaiRoute[];
  functionModuleLoaders: NavaiFunctionModuleLoaders;
...
export type ResolveNavaiMobileApplicationRuntimeConfigOptions = ResolveNavaiMobileRuntimeConfigOptions & {
...
export type ResolveNavaiMobileApplicationRuntimeConfigResult = ResolveNavaiMobileRuntimeConfigResult & {
  apiBaseUrl: string;
  env: NavaiMobileRuntimeEnv;
};

const DEFAULT_API_BASE_URL = "http://localhost:3000";
const DEFAULT_EMPTY_MODULE_LOADERS_WARNING =
  "[navai] No generated module loaders were found. Run your module loader generation command before starting the app.";
const API_URL_ENV_KEYS = ["NAVAI_API_URL"];
...
export function resolveNavaiMobileEnv(
  options: ResolveNavaiMobileEnvOptions = {}
): NavaiMobileRuntimeEnv {
  const sources = options.sources ?? [];

  return {
    NAVAI_API_URL: readFirstOptionalFromSources(sources, API_URL_ENV_KEYS),
    NAVAI_FUNCTIONS_FOLDERS: readFirstOptionalFromSources(sources, FUNCTIONS_ENV_KEYS),
    NAVAI_AGENTS_FOLDERS: readFirstOptionalFromSources(sources, AGENTS_ENV_KEYS),
    NAVAI_ROUTES_FILE: readFirstOptionalFromSources(sources, ROUTES_ENV_KEYS),
    NAVAI_REALTIME_MODEL: readFirstOptionalFromSources(sources, MODEL_ENV_KEYS)
  };
}

export async function resolveNavaiMobileApplicationRuntimeConfig(
  options: ResolveNavaiMobileApplicationRuntimeConfigOptions
): Promise<ResolveNavaiMobileApplicationRuntimeConfigResult> {
  const env = options.env ?? {};
  const moduleLoaderWarnings: string[] = [];

  if (Object.keys(options.moduleLoaders).length === 0) {
    moduleLoaderWarnings.push(options.emptyModuleLoadersWarning ?? DEFAULT_EMPTY_MODULE_LOADERS_WARNING);
  }

  const runtime = await resolveNavaiMobileRuntimeConfig({
  ...
  const apiBaseUrl =
    readOptional(options.apiBaseUrl) ??
    readFirstOptionalEnv(env, API_URL_ENV_KEYS) ??
    readOptional(options.defaultApiBaseUrl) ??
    DEFAULT_API_BASE_URL;

  return {
    ...runtime,
    warnings: [...moduleLoaderWarnings, ...runtime.warnings],
    apiBaseUrl,
    env
  };
}

export async function resolveNavaiMobileRuntimeConfig(
  options: ResolveNavaiMobileRuntimeConfigOptions
): Promise<ResolveNavaiMobileRuntimeConfigResult> {
  ...
  return {
    routes,
    functionModuleLoaders: primaryAgent?.functionModuleLoaders ?? functionModuleLoaders,
    agents,
    primaryAgentKey: primaryAgent?.key,
    modelOverride,
    warnings
  };
}
...
function resolveFunctionModuleLoaders(input: {
  ...
  const fallbackMatcher = createPathMatcher(input.defaultFunctionsFolder, agentFolders);
...
function readFirstOptionalFromSources(
  sources: Array<NavaiRuntimeEnvSource | undefined>,
  keys: string[]
): string | undefined {
  for (const source of sources) {
    const value = readFirstOptionalEnv(source, keys);
    if (value) {
      return value;
    }
  }
...
function readFirstOptionalEnv(env: NavaiRuntimeEnvSource | undefined, keys: string[]): string | undefined {
  if (!env) {
    return undefined;
  }
...
function readOptionalEnvValue(env: NavaiRuntimeEnvSource, key: string): string | undefined {
  const value = env[key];
