...
const SOURCE_FILE_PATH_LITERAL_PATTERN = /(["'`])((?:\.{0,2}\/)?src\/[^"'`\r\n]+\.[cm]?[jt]sx?)\1/g;
const DEFAULT_EXTENSIONS = [".ts", ".tsx", ".js", ".jsx", ".mts", ".mtsx", ".cts", ".ctsx", ".mjs", ".mjsx", ".cjs", ".cjsx"];
...
async function main() {
  ...
  const typeImport = args.typeImport ?? "@navai/voice-mobile";
  const exportName = args.exportName ?? "NAVAI_MOBILE_MODULE_LOADERS";

  const envFileValues = await readEnvFile(envFile);
  const configuredFunctionsFolders = readOptional(process.env.NAVAI_FUNCTIONS_FOLDERS) ?? readOptional(envFileValues.NAVAI_FUNCTIONS_FOLDERS);
  const configuredAgentsFolders = readOptional(process.env.NAVAI_AGENTS_FOLDERS) ?? readOptional(envFileValues.NAVAI_AGENTS_FOLDERS);
  const configuredRoutesFile = readOptional(process.env.NAVAI_ROUTES_FILE) ?? readOptional(envFileValues.NAVAI_ROUTES_FILE);
  ...
  const allSourceFiles = await walkDirectory(srcRoot, outputFile);
  const sourceByPath = new Map(allSourceFiles.map((absolutePath) => [sourcePathFromAbsolute(srcRoot, absolutePath), absolutePath]));

  const configuredFunctionTokens = functionsFolders
    .split(",")
    .map((token) => token.trim())
    .filter(Boolean);
  const functionTokens = configuredFunctionTokens.length > 0 ? configuredFunctionTokens : [defaultFunctionsFolder];
  const functionMatchers = functionTokens.map((token) => createPathMatcher(token, agentTokens));

  let selectedFunctionFiles = allSourceFiles.filter((absolutePath) => {
  ...
  if (selectedFunctionFiles.length === 0 && configuredFunctionTokens.length > 0) {
    ...
    selectedFunctionFiles = allSourceFiles.filter((absolutePath) =>
  ...
  const routesModuleFile = resolveModulePathFromMap(routesFile, sourceByPath);
  if (routesModuleFile) {
    selectedFiles.add(routesModuleFile);
    const routeReferencedFiles = await collectRouteReferencedFiles(routesModuleFile, sourceByPath);
    for (const absolutePath of routeReferencedFiles) {
  ...
  console.log(
    `[navai] Generated ${entries.length} mobile module loader(s) at ${normalizePath(path.relative(projectRoot, outputFile))}.`
  );
}
...
function printHelp() {
  const help = [
    ...
    "",
    "Usage:",
    "  navai-generate-mobile-loaders [--project-root <path>] [--env-file <path>]",
    "",
    "Options:",
    ...
    "  --type-import <module>              Type import path (default: @navai/voice-mobile)",
    "  --export-name <identifier>          Export constant name (default: NAVAI_MOBILE_MODULE_LOADERS)",
    "  --help                              Show this help"
  ].join("\n");
...
function buildModuleCandidates(inputPath) {
  ...
  return [...DEFAULT_EXTENSIONS.map((extension) => `${normalized}${extension}`), ...DEFAULT_EXTENSIONS.map((extension) => `${normalized}/index${extension}`)];
}

function resolveModulePathFromMap(inputPath, sourceMap) {
...
async function collectRouteReferencedFiles(routesAbsolutePath, sourceMap) {
  const contents = await readFile(routesAbsolutePath, "utf8");
  const matches = [...contents.matchAll(SOURCE_FILE_PATH_LITERAL_PATTERN)].map((match) => ensureSrcPrefix(match[2]));
  const uniqueMatches = [...new Set(matches)];
  const resolved = [];

  for (const sourcePath of uniqueMatches) {
    const found = resolveModulePathFromMap(sourcePath, sourceMap);
    if (found) {
      resolved.push(found);
      continue;
    }

    console.warn(`[navai] Route module reference was not found in src: "${sourcePath}".`);
  }
...
main().catch((error) => {
  ...
  console.error(`[navai] Failed to generate mobile module loaders: ${message}`);
  process.exitCode = 1;
});
