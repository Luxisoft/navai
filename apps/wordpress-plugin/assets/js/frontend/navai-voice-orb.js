...
${addLineNumbers(fragment)}`);
  ...
  // apps/wordpress-plugin/assets/js/src/navai-voice-orb.source.js
  var VERTEX_SHADER = (
    /* glsl */
    `
...
`
  ...
  var NavaiVoiceOrb = class {
    ...
    update(timeMs) {
      ...
      const deltaSeconds = (timeMs - this.lastTime) * 1e-3;
      this.lastTime = timeMs;
      this.program.uniforms.iTime.value = timeMs * 1e-3;
      this.program.uniforms.hue.value = this.getAnimatedHueValue(timeMs);
      this.program.uniforms.hoverIntensity.value = this.options.hoverIntensity;
      this.program.uniforms.backgroundColor.value = this.backgroundColorVec;
